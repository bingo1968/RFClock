...
#define		NODE_ACK_WAIT_TIME		0x80					//接收超时
//*****************************************************************************************
sbit 	GDO0	=P1^6;
sbit	MISO	=P1^3;
sbit	MOSI	=P1^1;
...
//************** COMM PROTOCOL 
#define PDA_CNT 0x10
INT8U xdata APTable[20];
INT8U xdata PDATable[PDA_CNT];
//************** UART Process 
#define SBUF_SIZE 32
INT8U xdata sUart[SBUF_SIZE];
INT8U xdata RFBuf[SBUF_SIZE];
INT8U data err=0;

INT8U data chnl = 0x00;
...
void SendUartBytes(INT8U *s, INT8U len)
{
	INT8U i;
	for (i=0; i<len; i++) SendUartByte(s[i]);
}
...
void Uart_Isr() interrupt 4 using 1
{
    if (RI)
    {
        ...
        P0 = SBUF;          //P0 show UART data
				...
				if (sUart[0]<SBUF_SIZE){
					sUart[sUart[0]+1]=SBUF;
					sUart[0]++;
					} 
		}
    if (TI)
...
INT8U UartDataOK(void)
{
	if ((sUart[0]>=sUart[1]) && (sUart[0])) return 1; 
	return 0;
}
...
//*****************************************************************************************
//函数名：int halRfSendPacketAddr(INT8U *txBuffer, INT8U size, INT8U addr)
//输入：发送的缓冲区，发送数据个数， 目的地址
//输出：
//功能描述：CC1100发送一组数据
//*****************************************************************************************
int halRfSendPacketAddr(INT8U *txBuffer, INT8U size, INT8U addr)
{
	unsigned int i=0;
	PAC=1;	//开功放
	halSpiWriteReg(CCxxx0_TXFIFO, size);		//input length of data to fifo buffer
	halSpiWriteReg(CCxxx0_TXFIFO, addr);		//dest to fifo
...
INT8U halRfReceivePacket(INT8U *rxBuffer, INT8U *length) 
{
    INT8U status[2];
    INT8U packetLength;
	INT8U i=(63)*4;  // 具体多少要根据datarate和length来决定
    ...
    if ((halSpiReadStatus(CCxxx0_RXBYTES) & BYTES_IN_RXFIFO)) //如果接的字节数不为0
	{
        ...
        if (packetLength >0) // <= *length) 		//如果所要的有效数据长度小于等于接收到的数据包的长度
		{
...
void SetRfChannel(INT8U chn)
{
	halSpiWriteReg(CCxxx0_CHANNR, chn);
}

void SetRfAddr(INT8U addr)
{
...
{
	INT8U code *led_table;          // led table
	INT8U i;
